...
I work on instrumentation for enviornmental measurment, and how we can use this data in a meaningful way.

The system we used this last summer uses our openIoTsensor boards, and a variety of soil moisture sensors.

They were installed at City Park Nine Golf Course fairway-7. We installed all 7 moisture sensors within close proximity.

We used 2 dollars for DFrobot, 22 dollars for SoilWatch10, 25 dollars for Tinovi, and 135 dollars for EC5 sensors.
...
### Get hourly weather data for a range of days in one table
...
```{r}
dates <- seq(from = as.Date("2019-07-31"), to = as.Date("2019-08-2"), by=1)
...
**ISSUE:** I had to g-sub the seconds column to be zeros to match weather data times, and change the date format.
...
I also did some column removal, renamed unfreindly column names, and got rid of N/A values generated from missing sensors values casued by a weak cellular conection.
...
### Soil Watch-10 Sensor Combine with DFrobot and Weather Data
...
finalDataSet_long <- st_tin_EC_sw_df_weather_remv_na_dups
```
...
We also noticed that some sensors were more sensitive to temperature effecting readings(Shown in a graph below).

Jim, the golf course superintendent allowed us to have full control of the irrigation in the area.

We had two major irrigation differences, until about Aug 19th we went from fully-saturation to fully field dry. After that we had constant irrigation events every day.
...
```{r}
qplot(x = time,y = EC5_1,data = finalDataSet_long, main = "EC sensor VWC vs Time",
                                            geom = "line")
...
```{r}
plot = ggplot() +
geom_line(data = finalDataSet_long, aes(x = time, y = EC5_1), color = "blue", alpha=0.5) + 
  geom_line(data = finalDataSet_long, aes(x = time, y = EC5_4), color = "blue",alpha=0.5) + 
  geom_line(data = finalDataSet_long, aes(x = time, y = precipProbability*100), color = "steelblue", alpha=0.75) +
    ylab("VWC (vol water/ vol soil) and Precip Prob")+
    ggtitle("Volumetric Water Content and Precip Prob vs. Time") # for the main title
...
```{r}
plot2 = ggplot() +
geom_line(data = finalDataSet_long, aes(x = time, y = soilWatchSensor_2), color = "blue") +
  geom_line(data = finalDataSet_long, aes(x = time, y = soilTempC), color = "orange")+
  ylab("VWC(vol water/ vol soil) and Soil Temperature(c)")+
  ggtitle("Soil Moisture and Soil Temp vs Time")
...
```{r}
plot3 = ggplot() +
geom_line(data = finalDataSet_long, aes(x = time, y = temperature), color = "red") +
  geom_line(data = finalDataSet_long, aes(x = time, y = soilTempC), color = "orange")+
  ylab("Temperature(c)")+
  ggtitle("Air Temp and Soil Temp vs Time")
